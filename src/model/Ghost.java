...
public class Ghost extends Creature {
	...
	public Direction smartPotentialDirection(Pacman pacman)
	{
		...
		if ((position.getDown().isNavegable(this)) && (direction != Direction.UP)&&(pacmanPosition.getY()>=ghostPosition.getY())) {
			smartDirectionsAvailables.add(Direction.DOWN);
		}
		if ((position.getLeft().isNavegable(this)) && (direction != Direction.RIGHT)&&(pacmanPosition.getX()<=ghostPosition.getX())) {
			smartDirectionsAvailables.add(Direction.LEFT);
		}
		if ((position.getRight().isNavegable(this)) && (direction != Direction.LEFT)&&(pacmanPosition.getX()>=ghostPosition.getX())) {
			smartDirectionsAvailables.add(Direction.RIGHT);
		}
		if ((position.getUp().isNavegable(this)) && (direction != Direction.DOWN)&&(pacmanPosition.getY()<=ghostPosition.getY())) {
